...
public class TestConcurrentPool {
  ...
  private static int deviceCount = 10000;
  private static int sensorCount = 10000;
  private static boolean isAligned = false;
  private static int clinetCount = 50;
  ...
  public static void main(String[] args)
      throws IoTDBConnectionException, InterruptedException, IOException {
    ...
    while (true) { // 等待所有任务都执行结束
      if (pool.isTerminated()) { // 所有的子线程都结束了
        System.out.printf(
            "创建TS %d, 共耗时: %f s",
            databaseCount * deviceCount * sensorCount,
    ...
    try {
      ...
      if (this.isAligned) {
        ...
        session.createAlignedTimeseries(
            devicePrefix + deviceIndex, paths, tsDataTypes, tsEncodings, compressionTypes, null);
      ...
      out.println(
          Thread.currentThread().getName() + " " + this.host + " " + this.deviceIndex + " " + e);
    } finally {
      try {
        session.close();
      } catch (IoTDBConnectionException e) {
